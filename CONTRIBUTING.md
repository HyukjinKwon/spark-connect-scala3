...
Releases are published to Maven Central (via the Sonatype Central Portal) and to
GitHub Releases by the [`release` workflow](.github/workflows/release.yml), which
runs `sbt ci-release` when a version tag is pushed.
...
2. Create a PGP key, publish its public half to a keyserver, and export the private
   half: `gpg --armor --export-secret-keys <KEY_ID> | base64`.
